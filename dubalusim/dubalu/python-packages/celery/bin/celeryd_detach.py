...
OPTION_LIST = daemon_options(default_pidfile='celeryd.pid') + (
    ...
    Option('--interpreter',
           default=None, action='store', dest='interpreter',
           help="Command for the interpreter"),
)


def detach(path, argv, logfile=None, pidfile=None, uid=None, gid=None,
           umask=0, working_directory=None, fake=False, interpreter=None, app=None):
    fake = 1 if C_FAKEFORK else fake
    path = interpreter if interpreter else path
    with detached(logfile, pidfile, uid, gid, umask, working_directory, fake):
        try:
            os.execvp(path, [path] + argv)
        except Exception:
            if app is None:
