...
class MultiTool(object):
    ...
    def show(self, argv, cmd):
        p = NamespacedOptionParser(argv)
        self.with_detacher_default_options(cmd, p)
        self.say('\n'.join(
    ...
    def with_detacher_default_options(self, cmd, p):
        _setdefaultopt(p.options, ['--pidfile', '-p'], '%N.pid')
        _setdefaultopt(p.options, ['--logfile', '-f'], '%N.log')
        p.options.setdefault(
            '--cmd',
            '{0} {1} {2}'.format(cmd, 'worker', '--detach') if 'manage.py' in cmd else '-m {0}'.format(celery_exe('worker', '--detach')),
        )
